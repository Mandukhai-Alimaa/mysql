...
func (c *mysqlConnectionImpl) GetCatalogs(ctx context.Context, catalogFilter *string) (catalogs []string, err error) {
	...
	if err != nil {
		return nil, c.Base().ErrorHelper.WrapIO(err, "failed to query catalogs")
	}
	defer func() {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&catalog); err != nil {
			return nil, c.Base().ErrorHelper.WrapIO(err, "failed to scan catalog")
		}
		catalogs = append(catalogs, catalog)
	}

	if err := rows.Err(); err != nil {
		return nil, c.Base().ErrorHelper.WrapIO(err, "error during catalog iteration")
	}
...
func (c *mysqlConnectionImpl) GetDBSchemasForCatalog(ctx context.Context, catalog string, schemaFilter *string) (schemas []string, err error) {
	...
	if schemaFilter != nil {
		...
		if err != nil {
			return nil, c.Base().ErrorHelper.WrapInvalidArgument(err, "invalid schema filter pattern")
		}
		if !matches {
...
func (c *mysqlConnectionImpl) getTablesOnly(ctx context.Context, catalog string, schema string, tableFilter *string) (tables []driverbase.TableInfo, err error) {
	...
	rows, err := c.Db.QueryContext(ctx, queryBuilder.String(), args...)
	if err != nil {
		return nil, c.Base().ErrorHelper.WrapIO(err, "failed to query tables for catalog %s", catalog)
	}
	defer func() {
		err = errors.Join(err, rows.Close())
	}()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tableName, &tableType); err != nil {
			return nil, c.Base().ErrorHelper.WrapIO(err, "failed to scan table info")
		}
	...
	if err := rows.Err(); err != nil {
		return nil, c.Base().ErrorHelper.WrapIO(err, "error during table iteration")
	}
...
func (c *mysqlConnectionImpl) getTablesWithColumns(ctx context.Context, catalog string, schema string, tableFilter *string, columnFilter *string) (tables []driverbase.TableInfo, err error) {
	...
	rows, err := c.Db.QueryContext(ctx, queryBuilder.String(), args...)
	if err != nil {
		return nil, c.Base().ErrorHelper.WrapIO(err, "failed to query tables with columns for catalog %s", catalog)
	}
	defer func() {
		err = errors.Join(err, rows.Close())
	}()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, c.Base().ErrorHelper.WrapIO(err, "failed to scan table with columns")
		}
	...
	if err := rows.Err(); err != nil {
		return nil, c.Base().ErrorHelper.WrapIO(err, "error during table with columns iteration")
	}
